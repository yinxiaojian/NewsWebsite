...
function analyseVideoUrl() {
    var content;
    $.get('/video/content/' + groupId, function (result, status) {
        if( isSuccess(status) ){
            parsingRes(result);
            analyseComment();
        }else{
...
function parsingRes(data) {
    $("#video-url").attr('src', data);
}

function analyseComment() {
    $.get('/video/comment/'+groupId+'/'+groupId+'/'+offset, function (result, status) {
        if( isSuccess(status) ){
            offset+=10;
