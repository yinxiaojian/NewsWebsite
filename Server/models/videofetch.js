/*
* get video url with id
*/
var request = require('request');
var Base64 = require('js-base64').Base64;
var baseUrl = 'http://i.snssdk.com/video/urls/v/1/toutiao/mp4/';

exports.getVideoOriginalURL = function(id ,callback) {
    request('http://www.toutiao.com/a'+id+'/', function (error, response, body) {
        if (!error && response.statusCode == 200) {
            var videoId = getVideoId(body);
            var url = baseUrl+videoId+'?r='+Math.random().toString(10).substring(2);
            var cipherText = crc32(url.slice(url.indexOf('/video')));
            url = url + '&s=' + (cipherText>>>0);
            console.log(url);
            request(url, function (error, response, body) {
                if (!error && response.statusCode == 200) {
                    var jsonData = JSON.parse(body);
                    var mainUrl = jsonData.data.video_list.video_1.main_url;
...
function getVideoId(html) {
    ...
    var index1 = res[0].indexOf("'");
    var length = res[0].length;
    return res[0].slice(index1+1,length-2);
}
